...
class UserManagementView(APIView):
    """GET → { users, pending_invites, multi_role_policy, branchId, branchName }."""
    permission_classes = [IsAuthenticated, IsAdminOrSuperAdmin]

    def get(self, request) -> Response:
        branch = resolve_branch(request)
        tenant_id = request.user.tenant_id
        invites = list(list_pending_invites(tenant_id, branch_id=branch.pk))
        ...
        users = [
            ...
            for u in list_managed_users(tenant_id, branch_id=branch.pk)
        ]
